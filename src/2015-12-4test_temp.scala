...
object osm extends Serializable{
  ...
  def mulDoc( _w : Array[Svector], _doc : Sdoc ) : Svector = {
    _doc.sortWith(_._1 < _._1)
    require( _w.head.size >= _doc.last._1, "Dimision fault! --From Smatrix.mulDoc")
    val res = new Svector(_w.size)
    val t = (0 until res.size).map{
      i =>
        var aij = 0.0 
        (0 until _doc.size).map{
          j => 
            val tCol = _doc(j)._1 - 1
            aij += _w(i)(tCol) * _doc(j)._3
        }
        res(i) = aij
    }
    res
  ...
  def netMulDoc( _w : Array[Svector], _doc : Sdoc ) : Svector = {
    _doc.sortWith(_._1 < _._1)
    require( _w.head.size >= _doc.last._1 + 1, "Dimision fault! --From Smatrix.netMulDoc")
    val res = new Svector(_w.size)
    val t = (0 until res.size).map{
      i =>
        var aij = 0.0 
...
class SAutoEncoder(
  ...
  def UpdateW( inDoc : Sdoc, hideOut : Svector, errHid : Svector, errOutDoc : Sdoc){
    ...
    /// hidden layer : deltaW(j,i) = lr * in(i) * ho(j)
    val alpha = this.config.alpha
    val beta = this.config.beta
...
val path = "file:///home/hadoop/suanec/data/MTrick/"
val ds = readMTrick(false,path+"Train.data",10) 
val dsv = ds.values.map(x => x._2).collect
val conf = new NNConfig
conf.setSize(Array(ds.cols,90,ds.cols))
conf.setLr(0.01)
conf.setAlpha(1)
conf.setBeta(2)
...
val labels = sc.textFile("file:///home/hadoop/suanec/data/MTrick/Train.label").map( x=> x.toInt -1 ).collect
