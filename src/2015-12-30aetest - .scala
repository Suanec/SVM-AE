...
val ds = readMTrick(false)
val docs = ds.values.collect 
val aew = InitWeight(Array(ds.cols,130,ds.cols))
val doc = docs.head._2
val paewh = aew.head.copy
val paewl = aew.last.copy
// NNBP(aew,doc)
// (0 until 1000).map( i => NNBP(aew,doc) )
...
def consPair[T,U]( arr : Array[T] ) : Array[(T,U)] = {
  // var res = new Array[(T,U)](arr.size)
  arr.map( x => (x,x.toString) ).toArray
...
type Sdata = (Int, Sdoc)
type SDataSet = (Int, Int, RDD[Sdata])
type Smodel = (Array[SDM], Array[Svector])
type Stask = (SDM,Svector,SDM,Svector)
...
def randomArray( _size : Int, _seed : Long ) : Svector = {
  val rand = new util.Random(_seed)
  val res = new Svector(_size)
  for(i <- 0 until res.size) res(i) = rand.nextDouble
  res
}
 ...
 * 
 */
def randomArrayWithRange(
  _size : Int,
  _min : Double,
  _max : Double,
  _seed : Long) : Svector = {
  val res = randomArray(_size,_seed)
  res.map( i => i*(_max - _min) + _min )
}
...
def randomSDM(
  ...
  _min : Double = -0.4, 
  _max : Double = 0.4,
  _seed : Long = 20151230135817L) : SDM = {
  val data = randomArrayWithRange(_row*_col, _min, _max, _seed)
  val res = arrFillMatByRow(_row, _col, data)
  res
}
...
def InitWeight(
  ...
  _min   : Double = -0.4,
  _max   : Double = 0.4,
  _seed  : Long = 20151230105917L) : Array[SDM] = {

  val res = new Array[SDM](_size.size-1)
  for(i <- 0 until _size.size-1){
    res(i) = randomSDM(_size(i+1),_size(i),_min,_max,_seed)
  }
  res
}///   def InitWeight(
...
def InitAE(
  ...
  _min : Double = -0.4,
  _max : Double = 0.4,
  _seed : Long = 20151230140533L) : Smodel = {
  val ws = InitWeight(_size,_min,_max,_seed)
  val bs = new Array[Svector](ws.size)
  for(i <- 0 until bs.size){
    bs(i) = randomArrayWithRange(_size(i+1),_min,_max,_seed)
  }
  (ws,bs)
}
...
def rddModel(
  ...
  _max : Double = 0.4,
  _partitions : Int = 4,
  _seed : Long = 20151230143847L) : RDD[Smodel] = {
  val rddParam = (_size,_min,_max,_seed)
  val arr = Array.fill(_partitions)(rddParam)
  val rddModel = sc.parallelize(arr,_partitions)
                    .map( param => InitAE(param._1,param._2,param._3,param._4) )
  rddModel
}
